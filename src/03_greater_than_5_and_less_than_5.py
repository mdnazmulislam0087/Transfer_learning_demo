...
STAGE = "Transfer Learning update_greater_than_less_than_5s" ## <<< change stage name 
...
def update_greater_than_less_than_5s(list_of_labels):
...
def main(config_path):
    ...
    y_train_bin, y_test_bin, y_valid_bin = update_greater_than_less_than_5s([y_train, y_test, y_valid])
    ...
    curr_time= time.time()
    logging.info (f"Starting time of the training of{STAGE} is : {curr_time} ")
    ## Train the model
    ...
    logging.info (f"Finishing time of the training of{STAGE} is : {final_time} ")
    diff_time =  final_time - curr_time
    logging.info (f"Total time taken in {STAGE} is : {(final_time - curr_time)} ")

    ## save the base model - 
    model_dir_path = os.path.join("artifacts","models")
    model_file_path = os.path.join(model_dir_path, "greater_than_5_orless_than5s_model.h5")
